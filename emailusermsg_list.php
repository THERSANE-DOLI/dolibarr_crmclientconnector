...
/**
 *   	\file       emailusermsg_list.php
 *		\ingroup    crmclientconnector
 *		\brief      List page for emailusermsg
 */
...
require_once __DIR__.'/class/emailusermsg.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('crmclientconnector', 'emailusermsg', 'write');
	$permissiontodelete = $user->hasRight('crmclientconnector', 'emailusermsg', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("crmclientconnector")) {
	accessforbidden('Module crmclientconnector not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'EmailUserMsg';
	$objectlabel = 'EmailUserMsg';
	$uploaddir = $conf->crmclientconnector->dir_output;
...
$title = $langs->trans("EmailUserMsgs");
//$help_url = "EN:Module_EmailUserMsg|FR:Module_EmailUserMsg_FR|ES:Módulo_EmailUserMsg";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/crmclientconnector/emailusermsg_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-crmclientconnector page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/crmclientconnector/emailusermsg_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEmailUserMsgRef";
$modelmail = "emailusermsg";
$objecttmp = new EmailUserMsg($db);
